...
int main() {
    ...
    std::vector<uint8_t> machine_code {
        #ifdef __linux__
        0x48, 0xc7, 0xc0, 0x01, 0x00, 0x00, 0x00,   // Store the "write" system call number for Linux
        #elif __APPLE__
    ...
    uint8_t *mem = (uint8_t*) mmap(NULL, required_memory_size, PROT_READ| PROT_WRITE | PROT_EXEC, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
    if(mem == MAP_FAILED) {
