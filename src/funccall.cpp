...
struct MemoryPages {
    ...
    size_t pages = 0;       // number of memory pages requested from the OS
    ...
    MemoryPages(size_t pages_requested = 1) {
        ...
        mem = (uint8_t*) mmap(NULL, page_size * pages_requested, PROT_READ | PROT_WRITE |
    ...
    // Push a vector of uint8 numbers to the memory
    ...
    // Check if there is enough avaliable space to push some data to the memory
    ...
    void show_memory() {
        std::cout << "\nMemory content: " << position << "/" << pages * page_size
            << "bytes used\n";
        std::cout << std::hex;
...
int main() {
    ...
    // Push the call to the C++ function test (actually we push the 
